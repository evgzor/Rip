...
{	
	...
	if([self.metadata objectForKey:kMetadataAlbumArtistKey]) {
		...
		fileRef.ID3v2Tag()->addFrame(textFrame);
	...
	if([self.metadata objectForKey:kMetadataReleaseDateKey]) {
		...
		if(releaseDate) {
			...
			fileRef.ID3v2Tag()->setYear((size_t)[releaseDateComponents year]);
		}		
	}
	...
	if([self.metadata objectForKey:kMetadataLyricsKey]) {
		...
		fileRef.ID3v2Tag()->addFrame(lyricsFrame);
	}
	...
	// Write TXXX frames
	TagLib::ID3v2::UserTextIdentificationFrame *trackGainFrame = TagLib::ID3v2::UserTextIdentificationFrame::find(fileRef.ID3v2Tag(), "replaygain_track_gain");
	TagLib::ID3v2::UserTextIdentificationFrame *trackPeakFrame = TagLib::ID3v2::UserTextIdentificationFrame::find(fileRef.ID3v2Tag(), "replaygain_track_peak");
	TagLib::ID3v2::UserTextIdentificationFrame *albumGainFrame = TagLib::ID3v2::UserTextIdentificationFrame::find(fileRef.ID3v2Tag(), "replaygain_album_gain");
	TagLib::ID3v2::UserTextIdentificationFrame *albumPeakFrame = TagLib::ID3v2::UserTextIdentificationFrame::find(fileRef.ID3v2Tag(), "replaygain_album_peak");
	
	if(trackGainFrame)
		fileRef.ID3v2Tag()->removeFrame(trackGainFrame);
	
	if(trackPeakFrame)
		fileRef.ID3v2Tag()->removeFrame(trackPeakFrame);
	
	if(albumGainFrame)
		fileRef.ID3v2Tag()->removeFrame(albumGainFrame);
	
	if(albumPeakFrame)
		fileRef.ID3v2Tag()->removeFrame(albumPeakFrame);
	...
	if(trackGain) {
		...
		fileRef.ID3v2Tag()->addFrame(userTextFrame);
	}
	...
	fileRef.ID3v2Tag()->removeFrames("RVA2");
	if(trackGain) {
		...
		fileRef.ID3v2Tag()->addFrame(relativeVolumeFrame);
	}
	...
	if([self.metadata objectForKey:kAlbumArtFrontCoverKey]) {
		...
		pictureFrame->setPicture(TagLib::ByteVector((const char *)[imageData bytes], (size_t)imageData.length));

		fileRef.ID3v2Tag()->addFrame(pictureFrame);
	}
