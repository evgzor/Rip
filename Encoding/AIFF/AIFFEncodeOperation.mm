...
@implementation AIFFEncodeOperation
...
{	
	...
	TagLib::RIFF::AIFF::File fileRef([[self.outputURL path] fileSystemRepresentation], false);
	...
	if([self.metadata objectForKey:kMetadataReleaseDateKey]) {
		...
		if(releaseDate) {
			...
			fileRef.tag()->setYear((size_t)[releaseDateComponents year]);
		}		
	}
